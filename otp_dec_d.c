...
// This function will decrypt passed message.
...
char *decryptMsg(char msg[], int msgSize, char keyCode[]){
	//printf("SERVER: HI I AM GOING TO RETURN A DECRYPT MSG\n");
	...
	//printf("THE FINAL DECRYPT BUFFER STRING IS.. \"%s\"\n", buffer);
    return buffer;
}
...
/*
	...
	returnMessageSize = read(establishedConnectionFD, &message_size, sizeof(message_size));
	if(returnMessageSize > 0){
		//printf("SERVER2: the message size is %d\n", ntohl(message_size));
		curMsgSize = ntohl(message_size);
		// temp - 1
		//curMsgSize--;
		//printf("SERVER2: the current message size is %d\n", curMsgSize);
	}
	else{
		error("ERROR reading from socket");
	}
*/
    ...
    // i think i accidently tack on an extra charactter here and there.
	char* secret = decryptMsg(msgBuffer, f1msgsize - 1, keyBuffer);
	...
	// will try this way to close out the connection.
...
int main(int argc, char *argv[])
{
	...
	listen(listenSocketFD, 5); // Flip the socket on - it can now receive up to 5 connections - increasing just in case
...
// WHILE LOOP FOR CONTINUOUS PORT CONNECTION WILL GO HERE.
	while(1){
        ...
        /*** 
        FORK GOES HERE - from lecture and
        ****/
        spawnpid = fork();
        switch(spawnpid){
            case -1:
